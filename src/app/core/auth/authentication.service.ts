...
export class AuthenticationService implements AuthService {
	...
	console.log("encoded"+encoded);
		...
		// var headers = new Headers();
		// headers.append('Content-Type', 'application/x-www-form-urlencoded');
		// headers.append('access_token',encoded)

		let headers = new Headers({'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*','access_token':encoded });
		let options = new RequestOptions({headers: headers});

		
		
		var url = "http://13.58.109.134/users/signup";
		return this.https.post(url, data,options).map((res:Response)=>{
	  ...
	  public loginService() {
		...
		var url = "http://13.58.109.134/users/signin";
		return this.https.get(url,options).map((res:Response)=>{
		console.log("data "+JSON.stringify(res));
		 return res.json();
		}).catch(this.errorHandler);
	  }
	  ...
	  public logoutService() {
		...
		var url = "http://13.58.109.134/users/logout";
		return this.https.get(url,options).map((res:Response)=>{
		console.log("data "+JSON.stringify(res));
		 return res.json();
		}).catch(this.errorHandler);
