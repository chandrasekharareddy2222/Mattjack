...
export class LoginComponent implements OnInit, OnDestroy {
	public model: any = { username: 'kusuma', password: 'demo' };
	...
	submit() {
		...
		this.authService.loginService().subscribe(response => {
		...
		// 	this.authService.signUpService(this.model).subscribe(response => {
	...
	ngOnInit(): void {
		...
		if (!this.authNoticeService.onNoticeChanged$.getValue()) {
			...
			<strong>Your Name</strong> and password
	...
	validate(f: NgForm) {
		...
		this.errors = [];
		if (objectPath.get(f, 'form.controls.email.errors.username')) {
			this.errors.push(this.translate.instant('AUTH.VALIDATION.INVALID', {name: this.translate.instant('AUTH.INPUT.USERNAME')}));
		}
